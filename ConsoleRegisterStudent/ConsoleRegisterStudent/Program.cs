/* The task for this final project portion was to modify existing code to make it run as intended.
 * Changes made to this program is indicated in the comments by the editor listed below.
 * 
 * Editor: Ryan Hutton
 * SNHU IT 230 - Software Development with C#
 * 11/28/2019
 */
...
namespace ConsoleRegisterStudent
{
    class Program
    {
        ...
        {
            if (choice < 1 || choice > 7) // Changed "...choice > 70" to "choice > 7". 
                return -1;
            else if ((choice == firstChoice) || (choice == secondChoice) || (choice == thirdChoice)) // Changed the logical ANDs to logical ORs.
                return -2;
            else if (totalCredit == 9) // Changed "totalCredit > 9" to "totalCredit == 9".
                return -3;
            return 0;   // Changed return from -4 to 0.
        }
